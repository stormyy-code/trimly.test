...
import { Search, Trophy, Sparkles, Crown, Star, ArrowDownNarrowWide, SlidersHorizontal } from 'lucide-react';
...
const CustomerHome: React.FC<CustomerHomeProps> = ({ onSelectBarber, lang }) => {
  ...
  // FILTRIRANJE: Samo odobreni i koji NISU suspendirani (banned)
  const barbers = useMemo(() => {
    const raw = db.getBarbersSync();
    return raw.filter(b => {
      const u = users.find(user => user.id === b.userId);
      // Ako korisnik ne postoji u listi (još se učitava) ili je banned, sakrij ga
            ...
            <div className="flex-1 py-1 min-w-0 flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                   <Star size={10} className={`${barber.avgRating > 0 ? 'text-[#D4AF37] fill-[#D4AF37]' : 'text-zinc-800'}`} />
                   <span className="text-zinc-500 text-[9px] font-bold">
                     {barber.avgRating > 0 ? `${barber.avgRating.toFixed(1)} (${barber.reviewCount})` : 'Novi član'}
                   </span>
                </div>
                {barber.minPrice > 0 && (
