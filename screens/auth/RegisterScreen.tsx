...
import { ArrowLeft, KeyRound, RefreshCw, ShieldCheck, Loader2 } from 'lucide-react';
        ...
        <div className="w-full max-w-sm space-y-12 text-center flex flex-col items-center">
          ...
          <form onSubmit={handleVerifyOtp} className="w-full space-y-8">
            ...
            <input 
              ...
              className="w-full bg-black border border-white/10 rounded-2xl py-6 text-center text-4xl font-black tracking-[0.4em] text-[#D4AF37] outline-none"
              autoFocus
            />
            <Button type="submit" loading={loading}>POTVRDI KÔD</Button>
            <div className="flex flex-col gap-4">
               ...
               <button type="button" onClick={cancelVerification} className="text-zinc-600 text-[10px] font-black uppercase tracking-widest">ODUSTANI</button>
            </div>
          </form>
          ...
          <div className="p-6 bg-amber-500/5 border border-amber-500/10 rounded-3xl">
             <p className="text-[7px] font-black text-zinc-600 uppercase tracking-widest leading-relaxed">
               NAPOMENA: U Supabase postavkama (Authentication &rarr; Settings) prekidač "Confirm Email" MORA biti uključen.
             </p>
          </div>
        </div>
      </div>
      ...
      <div className="w-full max-w-xs space-y-10 py-10">
        ...
        <form onSubmit={handleRegister} className="space-y-6">
           ...
           <div className="space-y-4">
             ...
             {(role === 'barber' || inviteCode.length > 0) && <Input label="Pozivni Kôd" placeholder="KOD..." value={inviteCode} onChange={setInviteCode} />}
           </div>
           <Button type="submit" loading={loading}>PRIDRUŽI SE</Button>
        </form>
